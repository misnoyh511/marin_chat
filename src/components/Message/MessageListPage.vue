<template>
  <v-ons-page>
    <v-ons-toolbar>
      <div class="left">
        <div class="toolbar-heading">メッセージ</div>
      </div>
    </v-ons-toolbar>
    <v-ons-list>
      <v-ons-list-item v-for="item in this.$root.privateMessageListItems" :key="item.id" modifier="longdivider" class="longdivider">
        <div class="left">
          <img class="list-item__thumbnail" :src="`http://a1333.ml/img/profile/${item.uuid}.jpg`" @click="showProfileImageModalWindow(item.id)">
        </div>
        <div class="center" @click="showMessagePage(item.id, item.uuid)">
...
export default {
  ...
  methods: {
    showMessagePage: function (userId, uuid) {
      this.$root.activeTabIndex = this.$root.messageTabIndex
      while (this.$root.pageStack.length > 1) {
        this.$root.pageStack.pop()
      }
      this.$root.pageStack.push({
        extends: MessagePage,
        data () {
          return {
            toUserId: userId,
